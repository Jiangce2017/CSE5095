...
typedef struct _point{
	...
	//virtual _point& operator-(const _point &p){ //Overloaded minus operator for points
	//	x = -p.x; y = -p.y; w = -p.w;
	//	return *this;
	//};
	virtual _point& operator-(const _point &p)const{ //Overloaded minus operator for points (vector difference)
		_point result;
		result.x = this->x - p.x;
		result.y = this->y - p.y;
		result.w = this->w - p.w;
